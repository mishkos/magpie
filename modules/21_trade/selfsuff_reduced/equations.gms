...
*' If it equals 0, all demand enters the comparative advantage pool.
 ...
 q21_trade_reg(h2,k_trade)..
  ...
  sum(supreg(h2,i2),vm_supply(i2,k_trade) + v21_excess_prod(i2,k_trade))
 ...
 q21_trade_reg_up(h2,k_trade)..
  ...
  (sum(supreg(h2,i2),vm_supply(i2,k_trade) + v21_excess_prod(i2,k_trade))/sum(ct,i21_trade_bal_reduction(ct,k_trade)))
  $(sum(ct,f21_self_suff(ct,h2,k_trade) >= 1))
